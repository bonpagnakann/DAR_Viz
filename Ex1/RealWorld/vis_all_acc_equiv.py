...
def vis_bar_accuracy(dataset, scenario, equivalent):
    ...
    for p in person:
        ...
        for acc in acc_list:
            ...
            df["Method"] = df["Method"].replace("EWC_Replay", "EWC Replay")
            ...
            exemplar_size = [equivalent]
            # Create a dictionary to hold data for each task
            task_data = {task: [] for task in tasks}

            for task in tasks:
                for method in methods:
                    method_data = df[(df["Method"] == method) & (df["Exemplar_Size"] == exemplar_size[0])][task].values
            ...
            colors = ['#1f77b4', '#ff7f0e', '#9467bd', '#d62728', '#8c564b', '#2ca02c']
            ...
            # Customize the plot
            if acc == 'All_Acc':
                fig_name = "Task Accuracy for All Classes by Exemplar Size and Method"
            elif acc == 'New_Acc':
                fig_name = "Task Accuracy for New Classes by Exemplar Size and Method"
            elif acc == 'Old_Acc':
                fig_name = "Task Accuracy for Old Classes by Exemplar Size and Method"
