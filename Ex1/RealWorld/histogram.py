...
def process_task_file(input_file, output_file):
    ...
    for method in task_data['Method'].unique():
        ...
        if method.startswith("VAE"):  # For VAE-based methods, use only the method name
...
def plot_boxplot(input_csv, output_image, task_name, method_order, exemplar_size_order, acc_type):
    ...
    for method in method_order:
        if method.startswith("VAE"):  # VAE-based methods have no exemplar size
    ...
    sns.boxplot(data=melted_data, x="Method", y="Accuracy", hue="Method", palette="Set3")
    ...
    plt.xlabel("Method - Exemplar Size", fontsize=14)
...
def accuracy_histogram(dataset, scenario):
    ...
    for p in person:
        for acc in acc_list:
            ...
            method_order = [
                ...
                "VAE - Adapt", "VAE - BBox", "VAE - Filter", "VAE - GMM"
            ]

            exemplar_size_order = exemplar_size[1:]
            ...
            for task_file in task_files:
                ...
                if acc == 'All_Acc':
                    output_image = os.path.join(result_dir, f"Box_plot_Acc_{task_name}.png")
                elif acc == 'New_Acc':
                    output_image = os.path.join(result_dir, f"Box_plot_of_Acc_{task_name[:-1]}{int(task_name[-1])}.png")
                elif acc == 'Old_Acc':
                    output_image = os.path.join(result_dir, f"Box_plot_of_Acc_{task_name[:-1]}{int(task_name[-1])}.png")
